...
export default function HomePage() {
  ...
  const displayName =
    ...
    (user as any)?.user_metadata?.full_name ?? user?.email?.split('@')[0] ?? '안녕하세요';
  ...
  const quickLinks = [
    {
      ...
      desc: '필수 공지 확인',
      icon: '📌',
      href: quickBoards.announcement
        ? `/boards/${quickBoards.announcement.slug}`
        : '/boards?section=announcement',
    ...
    },
    {
      ...
      desc: '솔직하게 털어놓기',
      icon: '🎭',
      href: quickBoards.anonymous
        ? `/boards/${quickBoards.anonymous.slug}`
        : '/boards?section=anonymous',
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-blue-100 bg-gradient-to-br from-blue-50 via-white to-sky-50 p-6">
        ...
        <h1 className="mt-1 text-xl font-bold text-slate-950">
          {displayName}님, 오늘은 어떤 이야기를 나눠볼까요? 👋
        </h1>
        <p className="mt-1.5 text-sm leading-6 text-slate-500">
          업무, 정보 공유, 익명 고민, 아이디어 제안까지 자유롭게 참여해보세요.
        </p>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-bold text-slate-950">🆕 최신 글</h2>
          <Link href="/feed" className="text-xs font-medium text-blue-600 hover:text-blue-700">
            더보기 →
          </Link>
        </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-bold text-slate-950">📋 둘러볼 게시판</h2>
